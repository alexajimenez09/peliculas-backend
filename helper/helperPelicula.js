...
const helpersPeliculas = {
    ...
    existeTitulo: () => {
        return async (req, res, next) => {
            ...
            if (existe) {
                return res.status(401).json({ msg: `La Pelicula ya ha sido registrada` });
    ...
    existeSubTitulo: () => {
        return async (req, res, next) => {
            const existe = await Pelicula.findOne({ subtitulo: req.body.titulo });
            if (existe) {
                return res.status(401).json({ msg: `El subtitulo ya ha sido registrado` });
                //throw new Error(`El email ya está registrado`)
            }
        }
    },
